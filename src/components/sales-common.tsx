...
export function HalfCard({
  ...
  actual: number;
  frozen: boolean;
  checkpointPct: number; // 0.8 للنصف الأول، 0.85 للنصف الثاني
}) {
  const { tone, pct } = perfOf(actual, track);
  // Checkpoint: التارجيت يُحسب على 80% (النصف الأول) أو 85% (النصف الثاني)
  // والنسبة = المحقق ÷ قيمة الـ Checkpoint وليس التارجيت الكامل
  const cpTarget =
    ...
    <section className="hairline gradient-border rounded-2xl bg-card/90 p-4 sm:p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {frozen && (
            <span className="rounded-full border border-border bg-card-2/60 px-2 py-0.5 text-2xs text-subtle">
              ❄ Frozen
            </span>
          )}
          <PerfPill tone={tone} />
      ...
      {/* صف الـ Checkpoint: التارجيت على 80% (نصف 1) أو 85% (نصف 2) والنسبة عليه */}
