...
export function DepartmentView({ dep, compact = false }: { dep: Dep; compact?: boolean }) {
  ...
  const monthlyActual = hasDailyActuals
    ? Object.values(daily).reduce((sum, value) => sum + value, 0)
    : totals.result;
  const firstHalfActual = Object.entries(daily)
    .filter(([date]) => Number(date.slice(-2)) <= 15)
    .reduce((sum, [, value]) => sum + value, 0);
  const secondHalfActual = Object.entries(daily)
    .filter(([date]) => Number(date.slice(-2)) > 15)
    .reduce((sum, [, value]) => sum + value, 0);
  ...
  const trackTarget = calculateDailyTarget(monthlyTarget, period) * trackDay;
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryCard label="Daily target" value={formatNumber(calculateDailyTarget(monthlyTarget, period))} />
      </div>
...
export function DepartmentGroupView({
  ...
  const firstHalfActual = deps.reduce(
    (total, dep) =>
      total +
      Object.entries(departmentDailyActuals[period]?.[dep] ?? {})
        .filter(([date]) => Number(date.slice(-2)) <= 15)
        .reduce((sum, [, value]) => sum + value, 0),
    0,
  );
  ...
  const today = new Date().toISOString().slice(0, 10);
  const trackDay = getTrackDay(period, today);
  const trackTarget = calculateDailyTarget(summary.target, period) * trackDay;
  const todayActual = deps.reduce(
    (total, dep) =>
      total + (departmentDailyActuals[period]?.[dep]?.[today] ?? 0),
    0,
  );
  const tone = statusOf(achievement).tone;
  const numberClass =
    ...
    <div className="flex flex-col gap-5">
      <section className="hairline rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <SummaryCard
            label="Daily target"
            value={formatNumber(calculateDailyTarget(summary.target, period))}
          />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs opacity-80">
            Track target through day {day}; rate = today's actual minus target
          </div>
        </div>
...
function getTrackDay(period: PeriodId, today: string): number {
